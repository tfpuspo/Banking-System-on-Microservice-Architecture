...
import { Plus, ShieldCheck, ShieldAlert, Send, PiggyBank } from "lucide-react";
...
const DEPOSIT_MUTATION = `
  mutation Deposit($toAccountId: ID!, $amountMinorUnits: Int!, $currency: String!) {
    deposit(toAccountId: $toAccountId, amountMinorUnits: $amountMinorUnits, currency: $currency) {
      transactionId
      status
    }
  }
...
export default function DashboardPage() {
  ...
  async function handleDeposit(toAccountId: string, amountMinorUnits: number, currency: string) {
    const data = await authedGraphqlRequest<{ deposit: { transactionId: string; status: string } }>(
      DEPOSIT_MUTATION,
      { toAccountId, amountMinorUnits, currency }
    );
    setTransferStatus("PENDING");
    pollTransaction(data.deposit.transactionId);
  }
  ...
  return (
    <div className="flex">
      ...
      {showDeposit && (
        <DepositModal accounts={accounts} onClose={() => setShowDeposit(false)} onSubmit={handleDeposit} />
      )}
